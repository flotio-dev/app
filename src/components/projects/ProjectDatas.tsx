...
export default function ProjectDatas({ name, url, github, status, environment, description, lastUpdated }: ProjectDatasProps) {
  const theme = useTheme();
  const [editMode, setEditMode] = useState(false);
  const [form, setForm] = useState({ name, environment, description });
          ...
          <Box display="flex" flexDirection="column" gap={2}>
            ...
            <Box display="flex" alignItems="center" gap={2}>
              ...
              <TextField
                fullWidth
                size="small"
                value={editMode ? form.description : description}
            ...
            <Box display="flex" alignItems="center" gap={2}>
              ...
              <TextField
                fullWidth
                size="small"
                value={editMode ? form.environment : environment}
                InputProps={{ readOnly: !editMode }}
                sx={{ input: { color: theme.palette.text.primary } }}
